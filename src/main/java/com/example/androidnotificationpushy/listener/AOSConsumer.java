package com.example.androidnotificationpushy.listener;
...
import com.example.androidnotificationpushy.model.request.PushNotificationRequest;
...
@Service
@AllArgsConstructor
public class AOSConsumer {
    private static final Logger LOGGER = LogManager.getLogger(AOSConsumer.class);
...
//    webPushService.saveWebHistory(webPushHistory);
        PushyPushRequest pushyPushRequest = new PushyPushRequest(transactionHistoryDto.getAmount().toString() + "$");
        androidNotificationService.sendSamplePush(pushyPushRequest, transactionHistoryDto.getCustomerId());
    }
