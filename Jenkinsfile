pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = 'fintrack-aos-service'
        CONTAINER_NAME = 'fintrack-aos-service-container'
    ...
    stages {
        ...
        //                     -Dsonar.projectKey=fintrack-aos-service \
        //                     -Dsonar.sources=src \
        ...
        stage('Trigger ManifestUpdate') {
            steps {
                script {
                    try {
                        build job: 'aos-service-pipeline-2', parameters: [string(name: 'DOCKERTAG', value: env.BUILD_NUMBER)]
                    } catch (Exception e) {
                        currentBuild.result = 'FAILURE'
